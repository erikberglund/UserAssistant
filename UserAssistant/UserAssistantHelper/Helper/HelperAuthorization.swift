...
class HelperAuthorization {
    ...
    static let authorizationRights = [
        HelperAuthorizationRight(command: #selector(HelperProtocol.groupAdminAdd(_:authData:completion:)),
                                 description: "Authenticate to become an Admin user.",
                                 ruleConstant: kAuthorizationRuleAuthenticateAsSessionUser)
    ]
